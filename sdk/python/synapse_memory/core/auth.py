...
logger = logging.getLogger("SynapseAuth")
...
class SecurityManager:
    ...
    def validate_api_key(
        ...
        with self._get_connection() as conn:
            cur = conn.cursor()
            cur.execute("SELECT * FROM api_keys WHERE key_hash = ?", (token_hash,))
            row = cur.fetchone()
            if not row:
            ...
            cur.execute("UPDATE api_keys SET last_used_at = ? WHERE key_id = ?", (now, record.key_id))
            conn.commit()
    ...
    def list_api_keys(self, organization_id: Optional[str] = None) -> List[Dict[str, Any]]:
        with self._get_connection() as conn:
            cur = conn.cursor()
            if organization_id:
                cur.execute("SELECT * FROM api_keys WHERE organization_id = ? ORDER BY created_at DESC", (organization_id,))
            else:
    ...
    def _log_audit(
        ...
        try:
            with self._get_connection() as conn:
                ...
                """, (log_id, time.time(), key_id, tenant_id, route, action, outcome, client_ip))
                conn.commit()
